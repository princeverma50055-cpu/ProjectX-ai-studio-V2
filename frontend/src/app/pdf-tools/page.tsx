...
export default function PDFToolsPage() {
  const [selectedTool, setSelectedTool] = useState<Tool>(pdfTools[0])
  ...
  async function processFile() {
    ...
    try {
      ...
      const res = await fetch(`${API}/api/pdf/${selectedTool.id}`, {
        method: 'POST',
        body: formData
      })
              ...
              <button
                ...
                className={`w-full text-left px-4 py-3.5 transition-all border-l-2 ${
                  selectedTool.id === tool.id
                    ? 'bg-amber-500/10 border-l-amber-500 text-white'
          ...
          <div className="lg:col-span-2 space-y-4">
                  ...
                  <div key={field.name}>
                    ...
                    {field.type === 'select' ? (
                      <select
                        ...
                        className="w-full px-4 py-3 glass rounded-xl border border-dark-500/40 text-white bg-transparent text-sm focus:outline-none focus:border-amber-500/50"
                      >
            ...
            <div
              ...
              className={`glass rounded-2xl border-2 border-dashed p-12 text-center cursor-pointer transition-all ${
                isDragActive
                  ...
                  : 'border-dark-500/60 hover:border-amber-500/40 hover:bg-amber-500/5'
              }`}
            >
              <input {...getInputProps()} />
              <Upload className="w-10 h-10 text-white/20 mx-auto mb-4" />
              <p className="text-white/60 font-medium mb-1">
                {selectedTool.multiple ? 'Drop multiple PDF files here' : 'Drop your PDF file here'}
              </p>
            ...
            {result?.pages && (
              <div className="glass rounded-xl border border-blue-500/20 p-5">
                <h4 className="text-sm font-semibold text-blue-400 mb-3">
                  Split Complete — {result.totalPages} pages
                </h4>
                <div className="grid grid-cols-4 gap-2">
                  {result.pages?.map((p: any) => (
                    <a
                      key={p.page}
                      href={`${API}${p.downloadUrl}`}
                      className="glass rounded-lg p-3 text-center text-sm text-white/60 hover:text-white border border-dark-500/30 hover:border-blue-500/30 transition-all"
                    >
                      Page {p.page}
                    </a>
                  ))}
            ...
            >
              {processing ? (
                ...
                <>
                  {selectedTool.label.split(' ').slice(1).join(' ')}
